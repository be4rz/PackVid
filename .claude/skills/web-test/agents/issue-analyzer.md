...
When a test fails, trace through the codebase to identify the exact cause and provide actionable fix recommendations. This enables rapid bug resolution without manual debugging.

## When Used

- Phase 5 of flow workflow (Bug Fixing Loop)
- After a test scenario fails during browser testing
...
1. **Failed test scenario**: The TC-XXX that failed
...
Failed test: TC-002 - Invalid email shows error
Expected: Error message "Invalid email format" appears below input
Actual: No error message shown, form appears to submit

Console errors: None

Screenshot: Shows form submitted without validation

Recent changes:
- Created LoginForm.tsx
- Added useLogin hook
- Styled with lohi-ui components

Codebase patterns:
- Forms use react-hook-form
- Validation with zod schemas
- Error display via FormMessage component
```
